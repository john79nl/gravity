...
{
    public class ReasoningRouter : IRouterService
    {
        ...
        private async Task<AgentResult> ExecuteStructuredAsync(string? agentName, AgentRequest? request, CancellationToken ct)
        {
            ...
            // Tool-name aliases: the model frequently says "file.read" / "file.write"
            // instead of "code_editor.read_file" / "code_editor.write_file"
            var toolAliases = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ["file"] = "code_editor",
                ["code_editor_file"] = "code_editor"
            };
            if (toolAliases.TryGetValue(targetAgent, out var aliasedTarget)
                && _agents.TryGetValue(aliasedTarget, out var aliasedAgent))
            {
                // Rewrite the verb if the model used a file.* verb name
                if (request.Verb.StartsWith("read", StringComparison.OrdinalIgnoreCase) && !request.Verb.Contains('_'))
                    request.Verb = "read_file";
                else if (request.Verb.StartsWith("write", StringComparison.OrdinalIgnoreCase) && !request.Verb.Contains('_'))
                    request.Verb = "write_file";
                else if (request.Verb.Equals("replace", StringComparison.OrdinalIgnoreCase)
                      || request.Verb.Equals("edit", StringComparison.OrdinalIgnoreCase))
                    request.Verb = "apply_diff";
                else if (request.Verb.Equals("list", StringComparison.OrdinalIgnoreCase)
                      || request.Verb.Equals("ls", StringComparison.OrdinalIgnoreCase))
                    request.Verb = "list_directory";

                return await aliasedAgent.ExecuteAsync(request, ct);
            }
            ...
            // Fuzzy agent name matching: check if the unknown name contains or is contained by a known agent
            var fuzzyMatch = _agents.Keys.FirstOrDefault(k =>
                k.Contains(targetAgent, StringComparison.OrdinalIgnoreCase) ||
                targetAgent.Contains(k, StringComparison.OrdinalIgnoreCase));
            if (fuzzyMatch != null && _agents.TryGetValue(fuzzyMatch, out var fallbackAgent))
            {
                return await fallbackAgent.ExecuteAsync(request, ct);
            }
